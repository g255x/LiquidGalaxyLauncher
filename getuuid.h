...
class MD5 {
	private:
		...
		uint32_t count[2];  // NUM
		uint8_t buffer[64]; // TEMP
		...
		// OPEN
		void decode(uint32_t* output, uint8_t* input, size_t len) {
			for (size_t i = 0, j = 0; j < len; i++, j += 4) {
	...
	public:
		...
		static void hash(const uint8_t* input, size_t length, uint8_t digest[16]) {
			MD5 md5;
...
/**
 MAKE UUID
 */
std::string getOfflineUUID(const std::string& username) {
	std::string combined = "GalaxyPlayer:" + username;

	// DO
	unsigned char digest[16];
	MD5::hash((const uint8_t*)combined.c_str(), combined.length(), digest);

	digest[6] = (digest[6] & 0x0F) | 0x30;  // V
	digest[8] = (digest[8] & 0x3F) | 0x80;  // RFC
